...
namespace GaussianSplatting.Runtime
{
    public class GaussianSplatAsset : ScriptableObject
    {
        ...
        // ── Codebook format support ──────────────────────────────────────────
        // Python converter writes m_PosFormat = 99 to identify codebook assets.
        // Unity serializes enums as int, so out-of-range value 99 persists fine.
        // When isCodebookFormat == true:
        //   posData   -> _pos.bytes  (float16 xyz, 6 bytes/splat)
        //   otherData -> _idx.bytes  (uint8 x4 codebook indices, 4 bytes/splat)
        //   colorData -> _cb.bytes   (codebook entries, 11264 bytes fixed)
        //   shData    -> null
        //   chunkData -> null
        public const int kCodebookFormatMagic = 99;
        public bool isCodebookFormat => (int)m_PosFormat == kCodebookFormatMagic;
