...
namespace GaussianSplatting.Runtime
{
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        GraphicsBuffer m_GpuCodebook;  // codebook format only
        ...
        internal static class Props
        {
            ...
            public static readonly int SplatCodebook = Shader.PropertyToID("_SplatCodebook"); // codebook format
        ...
        public bool HasValidAsset =>
            ...
            // shData is not used in codebook format
            (m_Asset.isCodebookFormat || m_Asset.shData != null);
        ...
        void CreateResourcesForAsset()
        {
            ...
            if (asset.isCodebookFormat)
            {
                // Codebook format: pos = float16 xyz (6 bytes/splat, padded to 4-byte boundary)
                //                 other = uint8 x4 indices (4 bytes/splat)
                // Use byte-level read to avoid size alignment issues
                var posBytes = asset.posData.GetData<byte>();
                m_GpuPosData = new GraphicsBuffer(GraphicsBuffer.Target.Raw | GraphicsBuffer.Target.CopySource, (posBytes.Length + 3) / 4, 4) { name = "GaussianPosData" };
                m_GpuPosData.SetData(posBytes);
                var otherBytes = asset.otherData.GetData<byte>();
                m_GpuOtherData = new GraphicsBuffer(GraphicsBuffer.Target.Raw | GraphicsBuffer.Target.CopySource, (otherBytes.Length + 3) / 4, 4) { name = "GaussianOtherData" };
            ...
            if (asset.isCodebookFormat)
            {
                // Codebook format: colorData = _cb.bytes (11264 bytes), no SH texture.
                // Upload codebook as a raw buffer; upload dummy 1-element SH buffer.
                m_GpuCodebook = new GraphicsBuffer(GraphicsBuffer.Target.Raw, (int)(asset.colorData.dataSize / 4), 4) { name = "GaussianCodebook" };
                m_GpuCodebook.SetData(asset.colorData.GetData<uint>());
                m_GpuSHData = new GraphicsBuffer(GraphicsBuffer.Target.Raw, 4, 4) { name = "GaussianSHData" };
                m_GpuColorData = Texture2D.blackTexture; // dummy, not used in codebook path
            }
            else
            {
                ...
                m_GpuColorData = tex;
                // Dummy codebook buffer so _SplatCodebook is always bound in compute shaders
                m_GpuCodebook = new GraphicsBuffer(GraphicsBuffer.Target.Raw, 4, 4) { name = "GaussianCodebookDummy" };
            }
        ...
        void SetAssetDataOnCS(CommandBuffer cmb, KernelIndices kernel)
        {
            ...
            uint format;
            if (m_Asset.isCodebookFormat)
                format = (uint)GaussianSplatAsset.kCodebookFormatMagic; // 99: signals codebook path in shader
            else
                format = (uint)m_Asset.posFormat | ((uint)m_Asset.scaleFormat << 8) | ((uint)m_Asset.shFormat << 16);
            // Always bind codebook buffer (dummy when not codebook format)
            cmb.SetComputeBufferParam(cs, kernelIndex, Props.SplatCodebook, m_GpuCodebook);
        ...
        internal void SetAssetDataOnMaterial(MaterialPropertyBlock mat)
        {
            ...
            if (m_Asset.isCodebookFormat)
            {
                format = (uint)GaussianSplatAsset.kCodebookFormatMagic;
                mat.SetBuffer(Props.SplatCodebook, m_GpuCodebook);
            }
            else
            {
                format = (uint)m_Asset.posFormat | ((uint)m_Asset.scaleFormat << 8) | ((uint)m_Asset.shFormat << 16);
        ...
        {
            // blackTexture is a built-in Unity texture, must not be destroyed
            if (m_GpuColorData != null && m_GpuColorData != Texture2D.blackTexture)
            ...
            DisposeBuffer(ref m_GpuCodebook);
        ...
        internal void SortPoints(CommandBuffer cmd, Camera cam, Matrix4x4 matrix)
        {
            ...
            int splatFmtForDist = m_Asset.isCodebookFormat
                ? GaussianSplatAsset.kCodebookFormatMagic
                : (int)m_Asset.posFormat;
            cmd.SetComputeIntParam(m_CSSplatUtilities, Props.SplatFormat, splatFmtForDist);
