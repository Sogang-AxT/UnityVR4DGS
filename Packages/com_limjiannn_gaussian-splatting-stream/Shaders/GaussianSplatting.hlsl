...
// Codebook format magic (matches GaussianSplatAsset.kCodebookFormatMagic)
#define VECTOR_FMT_CODEBOOK 99

// Codebook buffer layout (11264 bytes total):
//   [  0.. 3071] SH codebook:  256 x float3  (RGB color)
//   [3072.. 4095] Op codebook:  256 x float   (alpha)
//   [4096.. 7167] Sc codebook:  256 x float3  (scale xyz)
//   [7168..11263] Rot codebook: 256 x float4  (packed quaternion)
#ifdef SHADER_STAGE_COMPUTE
RWByteAddressBuffer _SplatCodebook;
#else
ByteAddressBuffer _SplatCodebook;
#endif
...
float3 LoadSplatPosValue(uint index)
{
    ...
    if (fmt == VECTOR_FMT_CODEBOOK)
    {
        // Codebook format: float16 x y z packed as 3x uint16 = 6 bytes/splat
        uint addr = index * 6;
        uint addrA = addr & ~3u;
        uint w0 = _SplatPos.Load(addrA);
        uint w1 = _SplatPos.Load(addrA + 4);
        uint raw0, raw1, raw2;
        if ((addr & 3) == 0) { raw0 = w0 & 0xFFFF; raw1 = w0 >> 16; raw2 = w1 & 0xFFFF; }
        else                 { raw0 = w0 >> 16;    raw1 = w1 & 0xFFFF; raw2 = w1 >> 16; }
        return float3(f16tof32(raw0), f16tof32(raw1), f16tof32(raw2));
    }
    uint stride = 0;
    if (fmt == VECTOR_FMT_32F)
        stride = 12;
...
SplatData LoadSplatData(uint idx)
{
    ...
    // ── Codebook format path ──────────────────────────────────────────
    if ((_SplatFormat & 0xFF) == VECTOR_FMT_CODEBOOK)
    {
        // _SplatPos   : float16 xyz  (6 bytes/splat)
        // _SplatOther : uint8 x4 indices [sh_idx, op_idx, sc_idx, rot_idx]  (4 bytes/splat)
        // _SplatCodebook: codebook entries (11264 bytes)

        // 1) Position (float16 xyz) - reuse LoadSplatPosValue which handles VECTOR_FMT_CODEBOOK
        s.pos = LoadSplatPosValue(idx);

        // 2) Read 4 codebook indices from _SplatOther
        uint idxAddr  = idx * 4;        // 4 bytes per splat
        uint idxAddrA = idxAddr & ~3u;
        uint idxWord  = _SplatOther.Load(idxAddrA);
        // indices are stored as uint8: byte0=SH, byte1=Op, byte2=Sc, byte3=Rot
        uint shIdx  = (idxWord >>  0) & 0xFF;
        uint opIdx  = (idxWord >>  8) & 0xFF;
        uint scIdx  = (idxWord >> 16) & 0xFF;
        uint rotIdx = (idxWord >> 24) & 0xFF;

        // Codebook offsets (bytes):
        //   SH  : 0           + entry * 12  (float3 = 3x float32)
        //   Op  : 256*12=3072 + entry * 4   (float  = 1x float32)
        //   Sc  : 3072+256*4=4096 + entry * 12
        //   Rot : 4096+256*12=7168 + entry * 16  (float4 = 4x float32)
        uint cbSHBase  = shIdx  * 12;
        uint cbOpBase  = 3072u  + opIdx  * 4;
        uint cbScBase  = 4096u  + scIdx  * 12;
        uint cbRotBase = 7168u  + rotIdx * 16;

        // 3) Color (RGB float32 from SH codebook)
        float3 cb_color = asfloat(_SplatCodebook.Load3(cbSHBase));
        s.sh.col = half3(cb_color);

        // 4) Opacity (float32 from Op codebook)
        float cb_alpha = asfloat(_SplatCodebook.Load(cbOpBase));
        s.opacity = half(cb_alpha);

        // 5) Scale (float3 from Sc codebook)
        float3 cb_scale = asfloat(_SplatCodebook.Load3(cbScBase));
        s.scale = cb_scale;

        // 6) Rotation (float4 PackSmallest3 from Rot codebook, decode to quaternion)
        float4 cb_rot = asfloat(_SplatCodebook.Load4(cbRotBase));
        s.rot = DecodeRotation(cb_rot);
